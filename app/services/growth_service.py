...
# How long a note stays "recently watered" after being created or edited.
WATERED_WITHIN_DAYS = 3
...
def was_recently_watered(updated_at, within_days=WATERED_WITHIN_DAYS):
    """A note counts as 'recently watered' if it was created or edited
    within the last few days - a lightweight stand-in for garden care."""
    if not updated_at:
        return False
    return (datetime.utcnow() - updated_at) <= timedelta(days=within_days)
